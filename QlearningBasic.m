...
%t1 is the temperature parameer from the softmax choice function 
...
for trialnum = 1:length(data) 
    ...
    pr1=;  exp(window1/t1)/(exp(window1/t1)+exp(window12/t1)); %softmax choice function for choosing option1 
    pr2=;   %fill in for choosing option 2

 
   if stimchoice==1
      trialfit(trialnum) = pr1; 
      window1 = ;  %%%%%%%%update EV for option 1 
   elseif stimchoice==2
      trialfit(trialnum) = pr2;
      window2 = ;   %%%%%%%%update EV for option 2 
   end
